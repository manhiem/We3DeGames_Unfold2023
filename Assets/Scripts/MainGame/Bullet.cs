...
public class Bullet : NetworkBehaviour
{
            ...
            foreach (var item in hits)
            {
                if (item.Hitbox != null)
                {
                    ...
                    if (didNotHitOurOwnPlayer && player.PlayerIsAlive)
                    {
                        ...
                        {
                            StartCoroutine(waitForIsDead(player));
                        }
    ...
    IEnumerator waitForIsDead(PlayerController player)
    {
        yield return new WaitForSeconds(.5f);

        Debug.LogError("Died or not" + player.GetComponent<PlayerHealthController>().GetIsDead());
        if (player.GetComponent<PlayerHealthController>().GetIsDead())
        {
            Object.GetComponent<PlayerHealthController>().SpawnTokens();
        }
    }
